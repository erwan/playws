...
object WS {
  ...
  class WSRequest(_method: String, _auth: Option[Tuple3[String, String, AuthScheme]], _calc: Option[SignatureCalculator]) extends RequestBuilderBase[WSRequest](classOf[WSRequest], _method, false) {
    ...
    private[playlibs] def execute: Promise[Response] = {
...
/*
    def executeStream[A](consumer: ResponseHeaders => Iteratee[Array[Byte], A]): Promise[Iteratee[Array[Byte], A]] = {
...
*/
     ...
     */
    def post(body: String, mimeType: String): Promise[Response] = prepare("POST", body, mimeType).execute
...
*/
     ...
     */
    def put(body: String, mimeType: String): Promise[Response] = prepare("PUT", body, mimeType).execute
...
*/
    ...
    private def prepare[T](method: String, body: String, mimeType: String = "text/plain") =
      new WSRequest(method, auth, calc).setUrl(url)
        .setHeaders(Map("Content-Type" -> Seq(mimeType)) ++ headers)
        .setQueryString(queryString)
        .setBody(body)
